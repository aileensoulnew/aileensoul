...
function GetArtPhotos() {
    $.ajax({
        type: 'POST',
        url: base_url + "artist/artistic_photos",
        //url: '<?php echo base_url() . "artist/artistic_photos" ?>',
        data: 'art_id=' + slug,
        beforeSend: function() {
            $('#loader').show();
...
function GetArtVideos() {
    $.ajax({
        type: 'POST',
        url: base_url + "artist/artistic_videos",
        //url: '<?php echo base_url() . "artist/artistic_videos" ?>',
        data: 'art_id=' + slug,
        beforeSend: function() {
            $('#loader').show();
...
function GetArtAudios() {
    $.ajax({
        type: 'POST',
        url: base_url + "artist/artistic_audio",
        //url: '<?php echo base_url() . "artist/artistic_audio" ?>',
        data: 'art_id=' + slug,
        beforeSend: function() {
            $('#loader').show();
...
function GetArtPdf() {
    $.ajax({
        type: 'POST',
        url: base_url + "artist/artistic_pdf",
...
jQuery(document).ready(function($) {
    ...
    var options = {
        ...
        uploadProgress: function(event, position, total, percentComplete) {
            ...
            $('#myBtn3').prop('onclick', null).off('click');
        ...
        complete: function(response) {
            ...
            $('html, body').animate({
                scrollTop: $(".upload-image-messages").offset().top - 100
            }, 150);
        }
    };
    // Submit the form
...
$('.editpost').on('click', function() {
    ...
    document.getElementById('editpostdetailbox' + abc).style.display = 'block';
    document.getElementById('editpostsubmit' + abc).style.display = 'block';
    //alert('#editpostname' + abc);
    //alert('#editpostdesc' + abc);
    $('#editpostname' + abc).val(editposttitle);
    $('#editpostdesc' + abc).html(editpostdesc);
});
...
function followuser(clicked_id) { //alert(clicked_id);
    $.ajax({
        type: 'POST',
    ...
    if (product_fileInput == '' && product_trim == '' && des_trim == '') {
        ...
        for (var i = 0; i < fileInput.length; i++) {
            ...
            if (foundPresent == true) {
                ...
                if (foundPresent1 == true && fileInput.length <= 10) {} else if (fileInput.length > 10) {
...
function get_art_progress() {
    $.ajax({
        type: 'POST',
        url: base_url +'artist_live/get_art_progress',
        data: '',
        dataType: "JSON",
        success: function (data) {            
...
function set_progress(count_profile_value,count_profile)
{
    ...
    else
    {
        ...
        $("#profile-progress").show();                
        $("#progress-txt").html("<a href='"+art_slug+"/details'>Complete your profile to get connected with more people</a>.");   
    }
