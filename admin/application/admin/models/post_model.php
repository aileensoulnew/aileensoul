...
class Post_model extends CI_Model {
    ...
    public function get_all_post($offset = '',$limit = '')
    {
        ...
        foreach ($result_array as $key => $value) {            
            ...
            $this->db->select("*")->from("promoted_post");
            $this->db->where('post_id', $value['id']);                
            $query = $this->db->get();
            $promoted_data = $query->row_array();
            $result_array[$key]['is_promoted'] = $promoted_data['status'];
        }
        return $result_array;
    }
    ...
    public function get_promoted_posts()
    {
        $sql = "SELECT up.*,pp.id_promoted_post,pp.link_url, pp.status as promoted_status,pp.priority,pp.show_label FROM ailee_user_post up LEFT JOIN ailee_promoted_post as pp ON pp.post_id = up.id WHERE pp.status != '0' AND up.post_for != 'profile_update' AND up.post_for != 'cover_update' ORDER BY pp.modify_date DESC";
        $result_array =  $this->db->query($sql)->result_array();
        foreach ($result_array as $key => $value) {            
            if($value['user_type'] == '1')
            {                
