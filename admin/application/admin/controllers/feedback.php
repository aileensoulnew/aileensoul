...
class Feedback extends CI_Controller {
    ...
    public function general()
    {
        ...
        $this->data['offset'] = $offset;
        $data = 'id,feedback_email,feedback_desc,feedback_screenshot,status,created_date';
        $contition_array = array('status' => '1');
        $this->data['feedbackData'] = $this->common->select_data_by_condition('feedback_general', $contition_array, $data, $sortby, $orderby, $limit, $offset, $join_str = array(), $groupby = '');
        // This is userd for pagination offset and limoi End
        ...
        //This if and else use for asc and desc while click on any field start
        if ($this->uri->segment(3) != '' && $this->uri->segment(4) != '') {
            $this->paging['base_url'] = site_url("feedback/general" . $short_by . "/" . $order_by);
        } else {
            $this->paging['base_url'] = site_url("feedback/general");
        }
        ...
        $contition_array = array( 'status =' => '1');
        $this->paging['total_rows'] = count($this->common->select_data_by_condition('feedback_general', $contition_array, 'id'));
        $this->data['total_rows'] = $this->paging['total_rows'];
        $this->data['limit'] = $limit;
        $this->pagination->initialize($this->paging);
        $this->data['search_keyword'] = '';            
        $this->load->view('feedback/general_feedback', $this->data);
    }
    ...
    public function detail($id) 
    {
        ...
        $this->load->view('feedback/general_feedback_detail',$this->data);
    }
    ...
    {        
        ...
        if ($this->input->post('search_keyword'))
        {
            ...
            $data = 'id,feedback_email,feedback_desc,feedback_screenshot,status,created_date';
            $search_condition = "(feedback_email LIKE '%$search_keyword%' OR feedback_desc LIKE '%$search_keyword%')";
            $contition_array = array('status' => '1');
            $this->data['feedbackData'] = $this->common->select_data_by_search('feedback_general', $search_condition, $contition_array,$data, $sortby, $orderby, $limit, $offset);

            //This if and else use for asc and desc while click on any field start
            if ($this->uri->segment(3) != '' && $this->uri->segment(4) != '') {
                $this->paging['base_url'] = site_url("feedback/general_search/" . $sortby . "/" . $orderby);
            } else {
                $this->paging['base_url'] = site_url("feedback/general_search/");
            }
            ...
            $this->paging['total_rows'] = count($this->common->select_data_by_search('feedback_general', $search_condition, $contition_array, 'id'));
            //for record display
            $this->data['total_rows'] = $this->paging['total_rows'];
            $this->data['limit'] = $limit;
            $this->pagination->initialize($this->paging);
    ...
    //clear search is used for unset session start
    public function clear_search_general()
    {
        if ($this->session->userdata('user_search_keyword'))
        {
            $this->session->unset_userdata('user_search_keyword');
            redirect('feedback/general','refresh');
        }
    }
    //clear search is used for unset session End
