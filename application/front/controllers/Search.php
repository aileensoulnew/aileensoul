...
class Search extends MY_Controller {
    ...
    public function ajax_freelancer_hire_search_OLD($searchkeyword = "", $searchplace = "") {
        // echo "rrrrr";die();
            ...
            foreach ($freelancerpostdata1 as $row) {
                ...
                if ($userid) {
                    if ($free_hire_result) {
                        ...
                        $return_html .= '<a style="margin-right: 4px;" href="' . base_url('freelance-employer/signup') . '" title="' . ucwords($row['freelancer_post_fullname']) . ' ' . ucwords($row['freelancer_post_username']) . '"><h6>';
                        $return_html .= ucwords($row['freelancer_post_fullname']) . ' ' . ucwords($row['freelancer_post_username']);
                        $return_html .= '</h6>
                        </a>
                        </li>';
                ...
                if ($userid) {
                    ...
                    if ($free_hire_result) {
                        ...
                        $return_html .= '<a href="' . base_url('freelance-employer/signup') . '"> Message </a>';
                        $return_html .= '<a href="' . base_url('freelance-employer/signup') . '"> Save </a>';
                    }
                } else {
    ...
    public function ajax_freelancer_hire_search($searchkeyword = "", $searchplace = "") {
        ...
        if (!empty($_GET["category_id"]) && $_GET["category_id"] != 'undefined') {
            $filter_array['category_id'] = $_GET["category_id"];
        }
        if (!empty($_GET["city_id"]) && $_GET["city_id"] != 'undefined') {
            $filter_array['city_id'] = $_GET["city_id"];
        }
        if (!empty($_GET["skill_id"]) && $_GET["skill_id"] != 'undefined') {
            $filter_array['skill_id'] = $_GET["skill_id"];
        }
        if (!empty($_GET["experience_id"]) && $_GET["experience_id"] != 'undefined') {
            $filter_array['experience_id'] = $_GET["experience_id"];
        }
        $start = ($page - 1) * $perpage;
        if ($start < 0)
            $start = 0;
        ...
        $cache_time = $this->db->get_where('cities', array('city_name' => $search_place))->row()->city_id;
        $freelancerpostdata1 = $this->common->get_search_results($search_skill,$search_place,$filter_array, $start, $perpage);
