...
// ARTISTICS PROFILE SLUG END
    ...
    public function art_post_insert($id = '', $para = '') {
        ...
        $return_html .= '<li><div class="post-design-product"><a class="buuis_desc_a" href="javascript:void(0);"  title="Category">';
        ...
        $return_html .= '</a></div></li><li></li> </ul></div>  
                                <div class="dropdown2">
        ...
        if ($row['posted_user_id'] != 0) {
            if ($this->session->userdata('aileenuser') == $row['posted_user_id']) {
                ...
                $return_html .= '<a onclick="deleteownpostmodel(' . $row['art_post_id'] . ')">
                                                ...
                                                <a href="' . base_url('artist/artistic_contactperson/' . $row['posted_user_id']) . '">
            ...
            if ($this->session->userdata('aileenuser') == $row['user_id']) {
                ...
                $return_html .= '<a onclick="deletepostmodel(' . $row['art_post_id'] . ')">
                                                ...
                                                <a href="' . base_url('artist/artistic_contactperson/' . $row['user_id']) . '">
    ...
    public function art_deletepost() {
        ...
        if ($profile_data[0]['status'] == '1' && $profile_data[0]['is_delete'] == '0') {
            ...
            if ($user_data['status'] == '1' && $user_data['is_delete'] == '0') {
                $return = 1;
            } else {
                $return = 0;
            }
    ...
    public function third_follow_user_data() {
        ...
        $geturl = $this->get_url($userlist['user_id']);
    ...
    public function insert_commentthreeimg() {
        ...
        foreach ($artcomment as $art_comment) {
            ...
            $cmtinsert .= '</div><div class="edit-comment-box"><div class ="inputtype-edit-comment">';
    ...
    public function delete_commenttwoimg() {
        ...
        if (count($artcomment) > 0) {
            foreach ($artcomment as $art_comment) {
                ...
                $cmtinsert .= '<span class="comment-edit-button"><button id="editsubmitimgtwo' . $art_comment['post_image_comment_id'] . '" style="display:none; margin-left:15px;" onClick="edit_commentimgtwo(' . $art_comment['post_image_comment_id'] . ')">Save</button></span></div></div><div class="art-comment-menu-design"> <div class="comment-details-menu" id="likecommentimg' . $art_comment['post_image_comment_id'] . '">';
    ...
    public function fourcommentimg($postid = "") {
        ...
        foreach ($artmulimage1 as $rowdata) {
            ...
            $fourdata .= '<span class="comment-edit-button"><button id="editsubmitimgtwo' . $rowdata['post_image_comment_id'] . '" style="display:none;" onClick="edit_commentimgtwo(' . $rowdata['post_image_comment_id'] . ')">Save</button></span></div></div>';
    ...
    public function likeuserlistimg() {
        ...
        foreach ($commnetcount as $comment) {
            ...
            $modal .= '<a href="' . base_url('artist/artistic_profile/' . $value) . '" title="' . $art_name1 . '" class="head_main_name" >';
...
//Get Job Seeker Name for title End
    ...
    public function search() {
        ...
        if ($searchskill == "") {
            ...
            $search_condition = "(art_post.art_post LIKE '%$searchskill%' or art_post.art_description LIKE '%$searchskill%'  or art_reg.designation LIKE '%$searchskill%' or art_reg.other_skill LIKE '%$searchskill%')";
        ...
        if ($this->session->userdata('aileenuser')) { //echo "h1"; die();
            $contition_array = array('user_id' => $this->session->userdata('aileenuser'), 'status' => '1', 'art_step' => '4');
            $artdata = $this->common->select_data_by_condition('art_reg', $contition_array, $data = '*', $sortby = '', $orderby = '', $limit = '', $offset = '', $join_str = array(), $groupby = '');
            if ($artdata) {
                $this->load->view('artist/recommen_candidate', $this->data);
            } else {
                $this->load->view('artist/user_search', $this->data);
            }
        } else {
            $this->load->view('artist/user_search', $this->data);
        }
    ...
    public function ajax_user_search() {
        ...
        if ($artpostdata) {
            ...
            <a title = "click to open"  href = "javascript:void(0)" target="_blank" onclick="register_profile();" alt="' . $artmultiimage[0]['file_name'] . '"><div class = "pdf_img">
                <img src="' . base_url('assets/images/PDF.jpg') . '" alt="PDF">
                    ...
                    foreach ($artmultiimage as $multiimage) {
                        ...
                        if ($userid) {
                            ...
                            $return_html .= '<a href="' . base_url('artist') . '"><div class="two-images">
                                                       <img class = "two-columns" src = "' . ART_POST_RESIZE1_UPLOAD_URL . $multiimage['file_name'] . '" alt="' . $multiimage['file_name'] . '">
                                                    </div> </a>';
                        } else {

                            $return_html .= '<a href="javascript:void(0);" onclick="register_profile();"><div class="two-images">
                                                       <img class = "two-columns" src = "' . ART_POST_RESIZE1_UPLOAD_URL . $multiimage['file_name'] . '" alt="' . $multiimage['file_name'] . '">
                                                    </div> </a>';
                        }
                    }
                    ...
                    if ($userid) {
                        ...
                        $return_html .= '<a href="' . base_url('artist') . '">';
                    } else {
                    ...
                    $return_html .= '<img src = "' . ART_POST_RESIZE2_UPLOAD_URL . $artmultiimage[3]['file_name'] . '" alt="' . $artmultiimage[3]['file_name'] . '">
                                                        </a>';
                    ...
                    $return_html .= '<div class="more-image" >
                                                <span>
                                                     View All (+' . (count($artmultiimage) - 4) . ')
                                                 </span></div>
                                             </a>
                ...
                if ($userid) {
                    ...
                    $return_html .= '<a class="ripple like_h_w" href="' . base_url('artist') . '">';
                } else {
                    $return_html .= '<a class="ripple like_h_w" href="javascript:void(0);" onclick="register_profile();">';
                }

                $return_html .= '<i class="fa fa-comment-o" aria-hidden="true"> 
                                                <span style="display: none;"></span>
                                                </i> 
                                                </a>
                                             </li>
                ...
                if ($artdata) {
                    ...
                    foreach ($artdata as $rowdata) {
                        ...
                        $return_html .= '<b title="' . ucfirst(strtolower($artname)) . '&nbsp;' . ucfirst(strtolower($artlastname)) . '">';
                ...
                <a href="javascript:void(0);" onclick="register_profile();">';
                        ...
                        if ($rowdata['user_id'] == $userid) {
                            ...
                            <div id="editcancle' . $rowdata['artistic_post_comment_id'] . '" style="display:none;">
                            <a id="' . $rowdata['artistic_post_comment_id'] . '" onClick="comment_editcancle(this.id)">Cancel</a></div></div>';
                        }
                        $userid = $this->session->userdata('aileenuser');
                        $art_userid = $this->db->select('user_id')->get_where('art_post', array('art_post_id' => $rowdata['art_post_id'], 'status' => 1))->row()->user_id;
        ...
        // no data avaloble code  start
        if (count($artuserdata) == 0 && count($artpostdata) == 0) {
            $return_html .= '<div class="profile_search" style="background-color: white; margin-bottom: 10px"><div class="text-center rio">
    ...
    public function artistic_notification_count($to_id = '') {
    ...
    public function pdf_display($id) {
        $this->data['title'] = "PDF | Artistic Profile" . TITLEPOSTFIX;
        $contition_array = array('post_id' => $id, 'is_deleted' => '1', 'insert_profile' => '1', 'post_format' => 'pdf');
