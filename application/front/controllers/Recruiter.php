...
class Recruiter extends MY_Controller {
	...
	public function add_post() {
		...
		$this->data['title'] = "Job Posting: Post Job and Hire Best Candidates".TITLEPOSTFIX;
...
//reactivate account start
	...
	public function recommen_candidate_post($id = "") {
		...
		if ($recruiter_deactive) {
			...
			*/
			...
			} elseif ($recommen_candid_totrec == 0 && $isfilterapply == false) {
			...
			if ($userid != $id && $id != '') {
				...
				if (count($rec_postdata) != '') {
					foreach ($rec_postdata as $post) {
						...
						$rec_post .= '<h5><a href="' . base_url() . substr($text,0,200). '-job-vacancy-in-' . $cityname . '-' . $post['user_id'] . '-' . $post['post_id'] . '">';
						$rec_post .= $post_name;
						$rec_post .= '</a></h5>';
						$rec_post .= '<p><a href = "' . base_url() . substr($text,0,200) .'-job-vacancy-in-' . $cityname . '-' . $post['user_id'] . '-' . $post['post_id'] . '">';
						...
						if (strlen($post['post_description']) > 150) {
							$rec_post .= '.....<a href="' . base_url() . '' . substr($text, 0,200) . $cityname . '-' . $post['user_id'] . '-' . $post['post_id'] . '">Read more</a>';
						}
						$rec_post .= '</p>
				...
				if (count($rec_postdata) != '') {
					...
					foreach ($rec_postdata as $post) {
						...
						$rec_post .= $post_name;
						$rec_post .= '</a></h5>';
						$rec_post .= '<p><a href = "' . base_url() . '' . substr($text, 0,200) . $cityname . '-' . $post['user_id'] . '-' . $post['post_id'] . '">';
						$rec_post .= $cache_time1;
						$rec_post .= '</a></p>';
						...
						if (strlen($post['post_description']) > 150) {
							$rec_post .= '.....<a href="' . base_url() . '' .substr($text, 0,200) . $cityname . '-' . $post['user_id'] . '-' . $post['post_id'] . '" target="blank();">Read more</a>';
						}
						$rec_post .= '</p>
	...
	public function recruiter_search_candidate_old() {
			...
			foreach ($savedata1 as $rec) {
				...
				if ($userdata[0]['status'] != 1) {
					...
					if ($rec['job_user_image'] != '') {
						$return_html .= '<a href="' . base_url() . 'job-profile/' . $rec['slug'] . '" title="' . $this->db->get_where('job_reg', array('user_id' => $rec['to_id']))->row()->fname . ' ' . $this->db->get_where('job_reg', array('user_id' => $rec['to_id']))->row()->lname . '">';
					...
					$return_html .= '<a class="post_name"  href="' . base_url() . 'job-profile/' . $rec['slug'] . '" title="' . $rec['fname'] . ' ' . $rec['lname'] . '">';
	...
	public function live_post($userid = '', $postid = '', $posttitle = '') {
		...
		if(isset($recdata) && empty($recdata))
		{
		...
		{			
			...
			$data = 'post_id,post_name,post_last_date,post_description,post_skill,post_position,interview_process,min_sal,max_sal,max_year,min_year,fresher,degree_name,industry_type,emp_type,rec_post.created_date,rec_post.user_id,recruiter.rec_firstname,recruiter.re_comp_name,recruiter.rec_lastname,recruiter.recruiter_user_image,recruiter.profile_background,recruiter.re_comp_profile,city,state,country,post_currency,salary_type,rec_post.comp_name,rec_post.comp_logo,rec_post.comp_profile';
			$contition_array = array('post_id' => $postid, 'status' => '1', 'rec_post.is_delete' => '0', 'rec_post.user_id' => $userid);
	...
	public function get_filer_data(){
		$limitstart = ($_POST['limitstart']) ? $_POST['limitstart'] : 0;
		$limit = ($_POST['limit']) ? $_POST['limit'] : 5;
		$result['job_title'] = $this->recruiter_model->get_job_title($limitstart,30);
		$result['cities'] = $this->recruiter_model->get_top_cities($limitstart,30);
		$result['job_industries'] = $this->recruiter_model->get_top_industries($limitstart,10);
		$result['job_skill'] = $this->recruiter_model->get_job_skill($limitstart,30);
		$result['job_experience'] = $this->recruiter_model->get_job_experience($limitstart,$limit);
	...
	public function recruiter_search_candidate() {
		...
		$candidatejob1 = $this->recruiter_model->get_recommen_candidate_search($searchkeyword,$searchplace,$city_id,$title_id,$industry_id,$skill_id,$experience_id,$userid,$page,$perpage);
