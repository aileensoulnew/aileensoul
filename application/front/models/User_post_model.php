...
class User_post_model extends CI_Model {
    ...
    public function postCommentDetail($comment_id = '') {
        $this->db->select("u.user_slug,u.user_gender,upc.user_id as commented_user_id,CONCAT(u.first_name,' ',u.last_name) as username, ui.user_image,ui.profile_background,upc.id as comment_id,upc.comment, upc.post_id, upc.created_date")->from("user_post_comment upc");
        $this->db->join('user u', 'u.user_id = upc.user_id', 'left');
        $this->db->join('user_login ul', 'ul.user_id = upc.user_id', 'left');
        $this->db->join('user_info ui', 'ui.user_id = upc.user_id', 'left');
        ...
        if($new_signup == 1)
        {
            if($getUserProfessionData)
            {
                ...
                $sql = "SELECT COUNT(*) as total FROM(
                        ...
                        WHERE upr.`user_id` = $user_id AND up.user_id != $user_id AND (upr.`designation` IN(c1.`opportunity_for`) OR $cond_sql) AND up.post_for = 'opportunity' ORDER BY up.created_date DESC) AS oppcon2
            ...
            if($getUserProfessionData)
            {
                $sql = "SELECT COUNT(*) as total FROM(
                    SELECT inner1.* FROM(
                        ...
                        ) as main1 WHERE main1.user_id != $user_id AND main1.status = 'publish' AND main1.is_delete = '0' AND main1.post_for != '' AND  main1.id NOT IN(SELECT post_id FROM ailee_user_post_delete WHERE user_id = $user_id) AND main1.created_date >= NOW() - INTERVAL 4 DAY
                    ) as inner1
                    UNION
                    SELECT inner2.* FROM(
                        SELECT main2.* FROM(
    ...
    public function userDashboardPost($user_id = '', $page = '') {
        ...
        foreach ($user_post as $key => $value) {
            ...
            $this->db->select("u.user_id,u.user_slug,u.user_gender,u.first_name,u.last_name,CONCAT(u.first_name,' ',u.last_name) as fullname,ui.user_image,ui.profile_background,jt.name as title_name,d.degree_name")->from("user u");
            $this->db->join('user_info ui', 'ui.user_id = u.user_id', 'left');
            $this->db->join('user_login ul', 'ul.user_id = u.user_id', 'left');
            $this->db->join('user_profession up', 'up.user_id = u.user_id', 'left');
            $this->db->join('job_title jt', 'jt.title_id = up.designation', 'left');
            ...
            $post_file_data = $query->result_array();
            $result_array[$key]['post_file_data'] = $post_file_data;
            $result_array[$key]['post_data']['total_post_files'] = count($post_file_data);

            $result_array[$key]['post_monetize'] = $this->common->is_post_monetize($value['id'],$userid_login);
            ...
            $result_array[$key]['post_comment_count'] = $this->postCommentCount($value['id']);
            $postCommentData = $this->postCommentData($value['id'],$userid_login);
    ...
    public function getUserSavedPost($user_id = '', $page = '') {
        ...
        $getDeleteUserPost = $this->deletePostUser($userid_login);
        $result_array = array();
        ...
        $this->db->where('ups.user_id', $userid_login);
        $this->db->where('up.status', 'publish');
        ...
        foreach ($user_post as $key => $value) {
            ...
            $result_array[$key]['page_data']['page'] = $page;
            $result_array[$key]['page_data']['total_record'] = $this->userPostCount($userid_login);
            $result_array[$key]['page_data']['perpage_record'] = $limit;
        }
       // echo '<pre>';
       // print_r($result_array);
    ...
    public function searchData($userid = '', $searchKeyword = '') {
        ...
        foreach ($user_post as $key => $value) {
            ...
            $result_array[$key]['post_monetize'] = $this->common->is_post_monetize($value['id'],$userid);
    ...
    public function searchDataProfileAjax($userid = '', $searchKeyword = '',$start = "",$limit = "5") {
        ...
        $this->db->select("u.user_id,u.first_name,u.last_name,u.user_gender,CONCAT(u.first_name,' ',u.last_name) as fullname,u.user_slug,ui.user_image,ui.profile_background,jt.name as title_name,d.degree_name,it.industry_name,up.city as profession_city,us.city as student_city,d.degree_name,un.university_name")->from("user u");
        $this->db->join('user_info ui', 'ui.user_id = u.user_id', 'left');
        $this->db->join('user_login ul', 'ul.user_id = u.user_id', 'left');
        $this->db->join('user_profession up', 'up.user_id = u.user_id', 'left');
        $this->db->join('job_title jt', 'jt.title_id = up.designation', 'left');
    ...
    public function searchDataPostAjax($userid = '', $searchKeyword = '',$start = "",$limit = "5") {
        ...
        foreach ($user_post as $key => $value) {
            ...
            $searchPostData[$key]['post_monetize'] = $this->common->is_post_monetize($value['id'],$userid);
            $searchPostData[$key]['user_like_list'] = $this->get_user_like_list($value['id']);
            $post_like_data = $this->postLikeData($value['id']);
            $post_like_count = $this->likepost_count($value['id']);
            $searchPostData[$key]['post_like_count'] = $post_like_count;
    ...
    public function get_user_like_list($post_id = '') {
        $this->db->select("upl.user_id,u.first_name,u.last_name,u.user_slug,CONCAT(u.first_name,' ',u.last_name) as fullname,u.user_gender,ui.user_image,ui.profile_background")->from("user_post_like upl");
        $this->db->join('user u', 'u.user_id = upl.user_id', 'left');
        $this->db->join('user_info ui', 'ui.user_id = upl.user_id', 'left');
        $this->db->join('user_login ul', 'ul.user_id = upl.user_id', 'left');
        $this->db->where('upl.post_id',$post_id);
    ...
    public function business_user_post_new($user_id = '', $page = '')
    {
        ...
        foreach ($user_post as $key => $value) {
            ...
            $result_array[$key]['post_comment_count'] = $this->postCommentCount($value['id']);
            $result_array[$key]['post_comment_data'] = $postCommentData = $this->postCommentData($value['id'],$userid_login);
    ...
    public function post_comment_reply_data($post_id = '',$comment_id = '',$user_id = '') {
        ...
        if(isset($post_comment_data) && !empty($post_comment_data))
        {
            foreach ($post_comment_data as $key => $value) {
                $post_comment_data[$key]['comment'] = nl2br($this->common->make_links($post_comment_data[$key]['comment']));
                $post_comment_data[$key]['comment_time_string'] = $this->common->time_elapsed_string(date('Y-m-d H:i:s', strtotime($post_comment_data[$key]['created_date'])));
    ...
    public function get_posts_data_total_rec($user_id = "",$search_hashtag = ""){
        $sql = "SELECT COUNT(*) as total_record FROM ailee_user_post up LEFT JOIN ailee_user_simple_post usp ON usp.post_id = up.id WHERE up.user_id != $user_id AND up.status = 'publish' AND up.is_delete = '0' AND up.post_for != '' AND up.post_for = 'simple' AND up.id NOT IN(SELECT post_id FROM ailee_user_post_delete WHERE user_id = $user_id)";

        $ht_sql = "";
        if($search_hashtag != undefined && !empty($search_hashtag))
        {
            $ht_sql = " AND usp.hashtag IN(SELECT id FROM ailee_hashtag WHERE ";
    ...
    public function get_posts_data($user_id = '',$page = '',$limit = '10',$search_hashtag = '')
    {
        ...
        $sql = "SELECT up.* FROM ailee_user_post up LEFT JOIN ailee_user_simple_post usp ON usp.post_id = up.id WHERE up.user_id != $user_id AND up.status = 'publish' AND up.is_delete = '0' AND up.post_for != '' AND up.post_for = 'simple' AND up.id NOT IN(SELECT post_id FROM ailee_user_post_delete WHERE user_id = $user_id)";
    ...
    public function get_article_data_total_rec($user_id = "",$search_field = '',$search_hashtag = ''){
        $sql = "SELECT COUNT(*) as total_record FROM ailee_user_post up LEFT JOIN ailee_post_article pa ON pa.id_post_article = up.post_id WHERE up.user_id != $user_id AND up.status = 'publish' AND up.is_delete = '0' AND up.post_for != '' AND up.post_for = 'article' AND up.id NOT IN(SELECT post_id FROM ailee_user_post_delete WHERE user_id = $user_id) ";

        $ht_sql = "";
        if($search_hashtag != undefined && !empty($search_hashtag))
        {
            $ht_sql = " pa.hashtag IN(SELECT id FROM ailee_hashtag WHERE ";
            foreach ($search_hashtag as $key => $value) {
                $str1 = str_replace(" ", "%' OR LOWER(hashtag) LIKE '%", strtolower($value->hashtag));
                $str1 = "LOWER(hashtag) LIKE '%".$str1."%'";
                $ht_sql .= $str1;
    ...
    public function get_article_data($user_id = '',$page = '',$limit = '10',$search_field = '',$search_hashtag = '')
    {
        $start = ($page - 1) * $limit;
        if ($start < 0)
            $start = 0;

        $sql = "SELECT up.* FROM ailee_user_post up LEFT JOIN ailee_post_article pa ON pa.id_post_article = up.post_id WHERE up.user_id != $user_id AND up.status = 'publish' AND up.is_delete = '0' AND up.post_for != '' AND up.post_for = 'article' AND up.id NOT IN(SELECT post_id FROM ailee_user_post_delete WHERE user_id = $user_id) ";
        ...
        $ret_arr = array(
            "article_post" => $result_array,
            "page" => $page
        );
        if($page == 1)
        {
            $total_record = $this->get_article_data_total_rec($user_id,$search_field,$search_hashtag);
            $ret_arr['total_record'] = $total_record;
        }
        return $ret_arr;
    }

    public function get_question_data_total_rec($user_id = "",$search_field = '',$search_hashtag = ''){
        $sql = "SELECT COUNT(*) as total_record FROM ailee_user_post up LEFT JOIN ailee_user_ask_question uaq ON uaq.post_id = up.id WHERE up.user_id != $user_id AND up.status = 'publish' AND up.is_delete = '0' AND up.post_for != '' AND up.post_for = 'question' AND up.id NOT IN(SELECT post_id FROM ailee_user_post_delete WHERE user_id = $user_id) ";

        $ht_sql = "";
        if($search_hashtag != undefined && !empty($search_hashtag))
        {
            $ht_sql = " uaq.hashtag IN(SELECT id FROM ailee_hashtag WHERE ";
            foreach ($search_hashtag as $key => $value) {
                $str1 = str_replace(" ", "%' OR LOWER(hashtag) LIKE '%", strtolower($value->hashtag));
                $str1 = "LOWER(hashtag) LIKE '%".$str1."%'";
                $ht_sql .= $str1;
    ...
    public function get_question_data($user_id = '',$page = '',$limit = '10',$search_field = '',$search_hashtag = '')
    {
        $start = ($page - 1) * $limit;
        if ($start < 0)
            $start = 0;

        $sql = "SELECT up.* FROM ailee_user_post up LEFT JOIN ailee_user_ask_question uaq ON uaq.post_id = up.id WHERE up.user_id != $user_id AND up.status = 'publish' AND up.is_delete = '0' AND up.post_for != '' AND up.post_for = 'question' AND up.id NOT IN(SELECT post_id FROM ailee_user_post_delete WHERE user_id = $user_id) ";
        ...
        $ret_arr = array(
            "question_post" => $result_array,
            "page" => $page
        );
        if($page == 1)
        {
            $total_record = $this->get_question_data_total_rec($user_id,$search_field,$search_hashtag);
            $ret_arr['total_record'] = $total_record;
        }
        return $ret_arr;
    }

    public function get_business_data_total_rec($user_id = '',$search_field = '',$search_city = '')
    {
    ...
    public function get_business_data($user_id = '',$page = '',$limit = '10',$search_field = '',$search_city = '')
    {
        $start = ($page - 1) * $limit;
        if ($start < 0)
            $start = 0;

        $sql = "SELECT bp.business_profile_id, bp.company_name, bp.country, bp.state, bp.city, bp.pincode, bp.address, bp.contact_person, bp.contact_mobile, bp.contact_email, bp.contact_website, bp.business_type, bp.industriyal, bp.details, bp.addmore, bp.user_id, bp.status, bp.is_deleted, bp.created_date, bp.modified_date, bp.business_step, bp.business_user_image, bp.profile_background, bp.profile_background_main, bp.business_slug, bp.other_business_type, bp.other_industrial, cr.country_name, ct.city_name, st.state_name, IF (bp.city != '',CONCAT(bp.business_slug, '-', ct.city_name),IF(st.state_name != '',CONCAT(bp.business_slug, '-', st.state_name),CONCAT(bp.business_slug, '-', cr.country_name))) as business_slug,IF(bp.industriyal = 0,bp.other_industrial,it.industry_name) as industry_name
            FROM ailee_business_profile bp
            LEFT JOIN ailee_user_login ul ON ul.user_id = bp.user_id
            LEFT JOIN ailee_industry_type it ON it.industry_id = bp.industriyal
            LEFT JOIN ailee_cities ct ON ct.city_id = bp.city
        ...
        $ret_arr = array(
            "business_data" => $result_array,
            "page" => $page
        );
        if($page == 1)
        {
            $total_record = $this->get_business_data_total_rec($user_id,$search_field,$search_city);
            $ret_arr['total_record'] = $total_record;
        }
        return $ret_arr;
    }
    ...
    public function get_hashtag_list($user_id = '', $page = "",$limit = "12",$search_tag = '') {
        ...
        $result_array = $query->result_array();*/
        foreach ($result_array as $key => $value) {
            $follower_counter = $value['follower_count'];//$this->get_hashtag_follower_count($value['id']);
    ...
    public function get_hashtag_recent_follower($hashtag_id = '', $user_id = '') {
        $sql = "SELECT u.user_slug,u.user_id,u.first_name,u.last_name,u.user_gender,ui.user_image,ui.profile_background FROM ailee_user u 
    ...
    public function get_hashtag_top_posts($hashtag_id = '', $user_id = '', $page = 0, $limit = '5'){        
        $start = ($page - 1) * $limit;
        if ($start < 0)
            $start = 0;
    ...
    public function get_hashtag_recent_posts($hashtag_id = '', $user_id = '', $page = 0, $limit = '5'){
        $start = ($page - 1) * $limit;
        if ($start < 0)
            $start = 0;
        ...
        $sql = "SELECT up.*
                ...
                LEFT JOIN ailee_post_article pa ON pa.id_post_article = up.post_id AND up.post_for = 'article'
                LEFT JOIN ailee_user_opportunity uo ON uo.post_id = up.id AND up.post_for = 'opportunity'
                WHERE up.status = 'publish' AND up.is_delete = '0' AND (FIND_IN_SET('".$hashtag_id."',us.hashtag) OR FIND_IN_SET('".$hashtag_id."',uaq.hashtag) OR FIND_IN_SET('".$hashtag_id."',pa.hashtag) OR FIND_IN_SET('".$hashtag_id."',uo.hashtag)) ORDER BY up.created_date DESC";
        if($limit != '') {
            $sql .= " LIMIT $start,$limit";
        }
        $query = $this->db->query($sql);
    ...
    public function get_hashtag_recent_posts_total_records($hashtag_id){
        $sql = "SELECT COUNT(*) as total_record FROM ailee_user_post up LEFT JOIN ailee_user_simple_post us ON us.post_id = up.id AND up.post_for = 'simple' LEFT JOIN ailee_user_ask_question uaq ON uaq.post_id = up.id AND up.post_for = 'question' LEFT JOIN ailee_post_article pa ON pa.id_post_article = up.post_id AND up.post_for = 'article' LEFT JOIN ailee_user_opportunity uo ON uo.post_id = up.id AND up.post_for = 'opportunity' WHERE up.status = 'publish' AND up.is_delete = '0' AND (FIND_IN_SET('".$hashtag_id."',us.hashtag) OR FIND_IN_SET('".$hashtag_id."',uaq.hashtag) OR FIND_IN_SET('".$hashtag_id."',pa.hashtag) OR FIND_IN_SET('".$hashtag_id."',uo.hashtag))";        
        $query = $this->db->query($sql);
        $result_array = $query->row_array();
        return $result_array['total_record'];
    }

    public function get_hashtag_opportunity_posts($hashtag_id = '', $user_id = '', $page = 0, $limit = '5'){
        $start = ($page - 1) * $limit;
        if ($start < 0)
            $start = 0;
        ...
        $sql = "SELECT up.*
                FROM ailee_user_post up 
                LEFT JOIN ailee_user_opportunity uo ON uo.post_id = up.id AND up.post_for = 'opportunity'
                WHERE up.status = 'publish' AND up.is_delete = '0' AND up.post_for = 'opportunity' AND FIND_IN_SET('".$hashtag_id."',uo.hashtag) ORDER BY up.created_date DESC";
        if($limit != '') {
            $sql .= " LIMIT $start,$limit";
        }
        $query = $this->db->query($sql);
    ...
    public function get_hashtag_opportunity_posts_total_records($hashtag_id){
        $sql = "SELECT COUNT(*) as total_record FROM ailee_user_post up LEFT JOIN ailee_user_opportunity uo ON uo.post_id = up.id AND up.post_for = 'opportunity' WHERE up.status = 'publish' AND up.is_delete = '0' AND up.post_for = 'opportunity' AND FIND_IN_SET('".$hashtag_id."',uo.hashtag)";        
        $query = $this->db->query($sql);
        $result_array = $query->row_array();
        return $result_array['total_record'];
    }

    public function get_hashtag_article_posts($hashtag_id = '', $user_id = '', $page = 0, $limit = '5'){
        $start = ($page - 1) * $limit;
        if ($start < 0)
            $start = 0;
        ...
        $sql = "SELECT up.*
                FROM ailee_user_post up
                LEFT JOIN ailee_post_article pa ON pa.id_post_article = up.post_id AND up.post_for = 'article'
                WHERE up.status = 'publish' AND up.is_delete = '0' AND up.post_for = 'article' AND (FIND_IN_SET('".$hashtag_id."',pa.hashtag)) ORDER BY up.created_date DESC";
        if($limit != '') {
            $sql .= " LIMIT $start,$limit";
        }
        $query = $this->db->query($sql);
    ...
    public function get_following_hashtags($user_id = '', $page = "",$limit = "12",$login_user_id='') {
