...
SELECT rp.* FROM ailee_job_reg jr, ailee_rec_post rp WHERE rp.post_name = jr.work_job_title AND jr.user_id = '".$userid."' AND jr.is_delete = '0' AND jr.status = '1' AND rp.is_delete = '0' AND rp.status = '1') as recommen_post ORDER BY recommen_post.post_id DESC";
    ...
    function latestJob($userid = "",$company_id = "",$category_id = "",$location_id = "",$skill_id = "",$job_desc = "",$period_filter = "",$exp_fil = "",$page = "",$limit = '5') {
        ...
        if($company_id != "")
        {
            $sql .= "r.rec_id IN (".$company_id.") AND ";
        }
        if($category_id != "")
        {
            $sql .= "rp.industry_type IN (".$category_id.") AND ";
        }
        if($location_id != "")
        {
            $sql .= "rp.city IN (".$location_id.") AND ";
        }
        if($skill_id != "")
        {
            $skill_id = str_replace(",", "|", $skill_id);
            $sql .= "rp.post_skill REGEXP '[[:<:]](".$skill_id.")[[:>:]]' AND ";
        }

        if($job_desc != "")
        {
            $sql .= "rp.post_name IN (".$job_desc.") AND ";
        }
        if($period_filter != "")
        {
            $sql_period = "";
    ...
    function get_job_search_new_result($userid = "",$job_keyword = "",$job_location = "",$work_time = "",$company_id = "",$category_id = "",$location_id = "",$skill_id = "",$job_desc = "",$period_filter = "",$exp_fil = "",$page = "",$limit = "5")
    {
        ...
        $this->db->select("post_id,post_name_txt as string_post_name,post_description,DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, city_name, country_name,min_year,max_year,fresher,CONCAT(rec_firstname,' ',rec_lastname) as fullname,user_id, rec_comp_name,rec_comp_logo, IF(city>0,city_name,IF(state>0,state_name,IF(country>0,country_name,''))) as slug_city,IF(comp_name != '',comp_name,rec_comp_name) as rec_comp_name,IF(comp_logo != '',comp_logo,rec_comp_logo) as comp_logo")->from('rec_post_search_tmp');
    ...
    {        
        ...
        $sql = "SELECT rj.post_id,rj.post_name,IFNULL(jt.name, rj.post_name) as string_post_name, rj.post_description, DATE_FORMAT(rj.created_date,'%d-%M-%Y') as created_date, ct.city_name,cr.country_name,rj.min_year,rj.max_year,rj.fresher,CONCAT(r.rec_firstname,' ',r.rec_lastname) as fullname,r.user_id, IF(rj.city>0,ct.city_name,IF(rj.state>0,st.state_name,IF(rj.country>0,cr.country_name,''))) as slug_city,IF(rj.comp_name != '',rj.comp_name,r.re_comp_name) as re_comp_name,IF(rj.comp_logo != '',rj.comp_logo,r.comp_logo) as comp_logo FROM (
    ...
    {
        $this->db->select("jr.fname, jr.lname, jr.email, jr.phnno, jr.dob, jr.gender, jr.country_id, jr.state_id, jr.city_id, jr.address, jr.job_user_image, jr.profile_background, jr.work_job_title, jr.field, jr.other_field, jt.name as work_job_title_txt, it.industry_name as field_txt,DATE_FORMAT(jr.dob, '%D %M %Y') as dob_txt,cr.country_name,st.state_name,ct.city_name,jr.experience,jr.exp_m,jr.exp_y,jr.job_active_status")->from("job_reg jr");
